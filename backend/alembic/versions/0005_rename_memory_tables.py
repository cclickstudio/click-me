...
- automation_runs 생성(3도메인 공용 자동화 워커 결과 저장소) — 005에 통합(멱등 IF NOT EXISTS).

빈 DB는 0001_baseline의 create_all이 개명 후 ORM대로 새 이름으로 바로 생성하므로,
기존 DB(옛 이름 존재)에서만 rename/drop이 실행되게 전부 멱등 가드.

주의: 이 마이그레이션이 먼저 DB에 적용됨(2026-07-03) — 0006_management_notifications가
이 리비전을 down_revision으로 가리킨다(체인: 0004 → 0005_rename → 0006_management).
"""
