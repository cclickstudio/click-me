# 시뮬레이션 KB 인제스천 — 마크다운 → 섹션 청크 → 임베딩 → simulation_kb_chunks
"""kb/*.md를 '## 섹션' 단위로 청크화해 임베딩 후 DB에 적재한다(소스별 재적재=멱등).

실행: cd backend && uv run python -m domain.simulation.assistant.kb_ingest
"""
...
from domain.simulation.assistant.retriever import EMBEDDING_MODEL
...
async def ingest() -> int:
    ...
    async with AsyncSessionLocal() as db:
        for md in sorted(_KB_DIR.glob("*.md")):
            ...
            await db.execute(delete(SimulationKbChunk).where(SimulationKbChunk.source == source))
