...
async def run_reaction(graph, persona: Persona, ad: AdInterpretation) -> PersonaReaction:
    ...
    상위 시뮬레이션 Trace 아래의 페르소나별 하위 Node로 묶인다
    (run_name=simulation:persona_react). 별도 Trace가 아니라 부모 컨텍스트에 자동 부착된다.
    """
    pid = getattr(persona, "id", None) or getattr(persona, "persona_id", None)
    config: dict = {
        "run_name": "simulation:persona_react",
        "tags": ["simulation", "react"],
        "metadata": {"domain": "simulation", "feature": "react"},
    }
    if pid:
