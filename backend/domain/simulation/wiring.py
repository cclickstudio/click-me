...
def _wrap_ssr(reactor: object) -> object:
    """SSR 배선(opt-in, 기본 OFF) — reactor를 SSRScoringReactor로 감싸 구매의도·신뢰도 재산정."""
...
def _build_reactor() -> object:
    ...
    SSR 점수화는 기본 OFF — SIMULATION_SCORING=ssr이면 체인 최외곽을 SSR로 감싼다(서술→임베딩 분포).
    """
...
def build_reaction_subgraph(settings=None, *, use_llm_qa=None):
    """반응+QA 재시도 서브그래프(컴파일본). 실 Gemini 반응(+GPT 폴백) + QA(§P4, mock 폴백 없음).
