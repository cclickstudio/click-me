...
class Panel(SimBase):
    ...
    grounding_meta: Mapped[dict] = mapped_column(JSONB, nullable=False)
...
class Persona(SimBase):
    ...
    ocean: Mapped[dict] = mapped_column(JSONB, nullable=False)
    media_behavior: Mapped[dict] = mapped_column(JSONB, nullable=False)
    consumption_values: Mapped[dict] = mapped_column(JSONB, nullable=False)
...
class AdAnalysis(SimBase):
    ...
    structured_analysis: Mapped[dict] = mapped_column(JSONB, nullable=False)
    ...
    mismatch_detail: Mapped[dict | None] = mapped_column(JSONB)
    model_version: Mapped[str] = mapped_column(String(50), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())
...
class Simulation(SimBase):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    ad_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False)
    ad_analysis_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False)
    panel_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False)
...
class PersonaReaction(SimBase):
    ...
    aisas: Mapped[dict] = mapped_column(JSONB, nullable=False)
