...
_TEMPLATE_COPY_GUIDE: dict[TemplateType, str] = {
    ...
    TemplateType.B: (
        "하단 단일 배너 레이아웃(이미지가 프레임 대부분을 차지). 헤드라인은 큰 글자로 "
        "긴급성·이벤트를 즉각 전달해야 하므로 짧고 임팩트 있게 (10자 이내 권장). "
        "본문은 보조 설명으로 최소화(1줄, 15자 이내 권장). "
        "CTA는 배너 우측에 크게 강조되므로 행동을 강하게 유도하는 짧은 문구로."
    ),
    TemplateType.C: (
