...
def build_sample_proposal(dx: DiagnosisResult) -> ActionProposal:
    ...
    proposal = ActionProposal(
        ...
        action_tier=ActionTier.TIER1,  # 잘못된 라벨 — 정책 판정은 Tier 3
        ...
        budget_before=DAILY_BUDGET_KRW,
        budget_after=int(DAILY_BUDGET_KRW * 1.5),
        max_total_spend=int(DAILY_BUDGET_KRW * 1.5) * 7,
    ...
    return proposal.model_copy(
        update={"proposal_hash": compute_proposal_hash(proposal_hash_payload(proposal))}
    )
...
def main() -> None:
    ...
    if requires_human(proposal.action_tier):
        budget_msg = f"₩{proposal.budget_before:,} → ₩{proposal.budget_after:,}"
