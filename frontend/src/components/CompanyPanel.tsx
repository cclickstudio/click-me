...
export default function CompanyPanel({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  ...
  // 회사별 그룹핑
  const grouped = projects.reduce<Record<string, { id: string; name: string }[]>>((acc, p) => {
  ...
  const filteredGrouped = search.trim()
    ? Object.fromEntries(
        Object.entries(grouped)
          ...
          .filter(([, projs]) => (projs as { id: string; name: string }[]).length > 0)
      )
    : grouped;
      ...
      <div className="flex-1 overflow-y-auto py-2 px-2 space-y-0.5">
        {loading ? (
          ...
          <CompanyItem
            key={name}
            name={name}
            projects={filteredGrouped[name] as { id: string; name: string }[]}
            activeSimId={activeSimId}
            activeGenId={activeGenId}
          />
        ))}
