    ...
    <div className="ml-4 space-y-0.5 mt-0.5">
      ...
      <div className="flex items-center gap-1.5 px-2 py-1">
        <button disabled={busy || trashed.length === 0} onClick={restoreAll}
          className="text-[10px] font-medium text-[#3182F6] hover:underline disabled:opacity-40 disabled:no-underline">전체 복원</button>
          ...
          {trashed.map((t) => (
            <div key={keyOf(t)} className="flex items-center gap-2 px-2 py-1 rounded-md hover:bg-[#F2F4F6] dark:hover:bg-[#252D3D] transition-colors">
              <label className="relative shrink-0 w-4 h-4 cursor-pointer">
                ...
                />
                <span className="block w-4 h-4 rounded-[5px] border border-[#D1D6DB] dark:border-[#3A4150] bg-white dark:bg-[#252D3D] peer-checked:bg-[#3182F6] peer-checked:border-[#3182F6] transition-colors" />
