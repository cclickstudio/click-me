...
const cardCls =
  'mt-1 w-full rounded-xl border border-line bg-card p-3';
...
function ReadRow({ domain, item }: { domain: Domain; item: ListItem }) {
  ...
  return (
    <div className="rounded-lg border border-line">
      <button
        onClick={toggle}
        className="w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-accent transition-colors rounded-lg"
      >
        ...
        <span className="text-[10px] text-ink-tertiary shrink-0">{open ? '접기' : '요약'}</span>
      </button>
      {open && (
        <div className="px-3 pb-3 pt-1 border-t border-line">
          {loading ? (
            <p className="text-xs text-ink-muted py-1">불러오는 중...</p>
          ) : summary?.error ? (
            <p className="text-xs text-ink-muted py-1">요약을 불러오지 못했어요.</p>
          ) : domain === 'sim' ? (
            <div className="grid grid-cols-2 gap-1.5 text-xs py-1">
              <span className="text-ink-tertiary">클릭 의향률 <b className="text-ink">{pct(summary?.click_intent_rate)}</b></span>
              <span className="text-ink-tertiary">구매의도 <b className="text-ink">{sc(summary?.purchase_intent)}/5</b></span>
              <span className="text-ink-tertiary">신뢰도 <b className="text-ink">{sc(summary?.trust_avg)}/5</b></span>
              <span className="text-ink-tertiary">거부율 <b className="text-ink">{pct(summary?.rejection_rate)}</b></span>
            </div>
          ) : (
            <p className="text-xs text-ink-secondary py-1">
  ...
  return (
    <>
            ...
            >
              <span
                className={`w-4 h-4 rounded border shrink-0 flex items-center justify-center text-[10px] ${
                  on ? 'bg-primary border-primary text-white' : 'border-[#B0B8C1]'
                }`}
              >
                {on ? '✓' : ''}
              </span>
      ...
      <button
        ...
        className="mt-2 w-full py-2 rounded-lg bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary-hover disabled:opacity-40 transition-colors"
      >
  ...
  if (!items || items.length === 0) {
    return (
      <div className={cardCls}>
        <p className="text-sm text-ink-tertiary">아직 {domain === 'sim' ? '시뮬레이션' : '광고 생성'} 내역이 없어요.</p>
      </div>
    );
  }
  ...
  return (
    <div className={cardCls}>
      <p className="text-sm font-semibold text-ink mb-2">
        ...
        <span className="text-[11px] font-normal text-ink-tertiary"> ({items.length})</span>
      </p>
      {mode === 'compare' ? (
        ...
        {mode === 'read'
          ...
          : items.map((it) => (
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-2 text-left rounded-lg border border-line hover:border-primary hover:bg-primary-subtle transition-colors"
              >
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-ink truncate">{it.title}</p>
                  <p className="text-[10px] text-ink-muted">
                    {it.status ?? ''}
                    {it.created_at ? ` · ${fmtDate(it.created_at)}` : ''}
                  </p>
                </div>
                <span className="text-xs text-primary font-semibold shrink-0">선택 →</span>
              </button>
            ))}
      </div>
      )}
