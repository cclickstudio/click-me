  ...
  return (
    <div className="h-screen flex flex-col items-center justify-center px-4 bg-white dark:bg-[#0F1117] transition-colors">
      <div className="w-full max-w-lg">
          <p className="text-[11px] font-semibold text-ink-tertiary uppercase tracking-wide mb-1">
            {projectName}
          </p>
          <h2 className="text-xl font-bold text-ink mb-5">채팅 시작</h2>

          <button
            onClick={() => router.push(`/chat/${projectId}/new`)}
            className="w-full flex items-center gap-3 p-4 rounded-xl bg-primary text-primary-foreground hover:bg-primary-hover transition-colors mb-5"
          >
          ...
          {sessions === undefined ? (
            <p className="text-sm text-ink-muted px-1 py-2">불러오는 중...</p>
          ) : sessions.length === 0 ? (
            <p className="text-sm text-ink-muted px-1 py-2">아직 채팅이 없어요. 새 채팅으로 시작하세요.</p>
          ) : (
            <div className="space-y-1.5 max-h-[40vh] overflow-y-auto">
              {sessions.map((s) => (
                <button
                  ...
                  className="w-full flex items-center gap-3 p-3 text-left rounded-xl border border-line hover:border-primary hover:bg-primary-subtle transition-colors"
                >
                  <span className="w-7 h-7 shrink-0 flex items-center justify-center rounded-lg bg-primary-subtle text-primary">
