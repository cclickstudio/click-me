...
type Row = { id: string; ad_id: string; status: string; sample_size: number; created_by_name: string | null; created_at: string };
...
const statusLabel: Record<string, string> = {
  COMPLETED: '완료', QUEUED: '대기', RUNNING: '진행 중', FAILED: '실패',
};

export default function CompanySimulationsPage() {
  const [list, setList] = useState<Row[]>([]);
  const [loading, setLoading] = useState(true);

  const load = () => {
    setLoading(true);
    fetch(`${API_BASE}/api/company/simulations`, {
    ...
    <AppLayout>
      <div className="px-8 py-8 max-w-5xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-sm text-[#8B95A1] dark:text-[#6B7280] mt-1">우리 조직의 광고 시뮬레이션 실행 목록</p>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#F2F4F6] dark:border-[#252D3D] bg-[#F9FAFB] dark:bg-[#252D3D]">
                  <th className="text-left px-6 py-3 text-xs font-semibold text-[#8B95A1]">상태</th>
