  ...
  return (
    <div className="px-8 py-8 max-w-5xl mx-auto space-y-8">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-ink">팀 관리</h1>
          <p className="text-sm text-ink-tertiary mt-1">팀을 만들고, 카드를 드래그해 팀원을 배정하세요. 계정 생성·수정은 ‘직원 관리’에서 하세요.</p>
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <p className="text-sm font-semibold text-ink">팀 배정</p>
          {addingTeam ? (
            <div className="flex items-center gap-2">
              <input autoFocus value={newTeam} onChange={(e) => setNewTeam(e.target.value)}
                ...
                placeholder="팀 이름"
                className="px-3 py-2 text-sm border border-line rounded-xl bg-surface-2 text-ink placeholder:text-ink-muted focus:outline-none focus:border-primary" />
              <button onClick={createTeam} className="px-3 py-2 text-sm font-medium bg-primary text-primary-foreground rounded-xl hover:bg-primary-hover transition-colors">추가</button>
              <button onClick={() => { setAddingTeam(false); setNewTeam(''); }} className="px-3 py-2 text-sm text-ink-tertiary rounded-xl hover:bg-accent transition-colors">취소</button>
            </div>
          ) : (
            <button onClick={() => setAddingTeam(true)}
              className="px-3 py-2 text-sm font-medium text-primary border border-primary/30 rounded-xl hover:bg-primary-subtle transition-colors">+ 팀 추가</button>
          )}
        </div>
              ...
              return (
                <div key={col.key}
                  ...
                  <div className="flex items-center justify-between px-4 py-3 border-b border-line">
                    ...
                    {col.teamId && (
                      <button onClick={() => deleteTeam(col.teamId!, col.name)} title="팀 삭제"
                        className="shrink-0 p-1 rounded-md text-ink-muted hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
                  ...
                  <div className="p-2 space-y-2 min-h-[120px]">
                    {cards.length === 0 ? (
                      <p className="text-xs text-ink-muted text-center py-6">여기로 드래그</p>
                    ) : cards.map((m) => (
                      <div key={m.member_id} draggable
                        ...
                        <p className="text-sm font-medium text-ink truncate">{m.user_name}</p>
                        <p className="text-[11px] text-ink-tertiary truncate">{m.login_id}</p>
                      </div>
                    ))}
                  </div>
                </div>
