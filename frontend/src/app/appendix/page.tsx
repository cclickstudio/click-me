...
// 상단에는 포털사이트 스타일의 큰 검색창을 둔다. 전체화면 버튼은 페이지 전체(문서 루트)를
// Fullscreen API로 확대한다 — 전체화면 중에는 페이지 폭·여백을 줄여 화면을 꽉 채운다.
...
export default function AppendixPage() {
  ...
  // 카드가 아니라 페이지 전체(문서 루트)를 전체화면으로 확대한다 — 초기 화면·자료 패널 공용.
  const enterPageFullscreen = async () => {
    setFullscreen(true);
    try {
      await document.documentElement.requestFullscreen();
    } catch {
      // Fullscreen API가 막힌 환경 — 상태만 켜져도 닫기 버튼으로 복귀 가능.
    }
  };
  ...
  return (
    <div
      className={
        fullscreen
          ? 'flex min-h-screen w-full flex-col justify-center px-5 py-4' // 남는 세로 공간을 위아래로 균등 배분
          : big
            ? 'mx-auto w-[90%] py-4'
        ...
        <div className="min-w-0 lg:sticky lg:top-8 lg:self-start">
          {activeDomain && activeQuestion && activeExhibit ? (
            <div
              className={
                big
                  ? `rounded-2xl border-2 ${fullscreen ? 'p-6' : 'p-10'} ${TONE_CLASS[activeDomain.accent].border} bg-card`
            ...
            <div className="relative rounded-2xl border-2 border-line bg-card p-4">
              {fullscreen ? (
                <button
                  type="button"
                  onClick={exitFullscreen}
                  aria-label="전체화면 닫기"
                  title="전체화면 닫기"
                  className="absolute right-3 top-3 z-10 rounded-lg border border-line bg-card p-2 text-ink-tertiary transition-colors hover:bg-accent hover:text-ink"
                >
                  <X size={16} strokeWidth={1.8} />
                </button>
              ) : (
