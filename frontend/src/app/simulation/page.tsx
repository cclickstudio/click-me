...
export default function SimulationRunPage() {
        ...
        <div className="px-8 py-8 max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-[1fr_1fr] gap-5 items-start">
            ...
            <div className={`${cardCls} flex flex-col gap-5`}>
              <p className="text-sm font-semibold text-[#191F28] dark:text-[#F2F4F6]">광고 입력</p>
  ...
  if (step === "result" && result) {
    ...
    const personaMap = new Map((result.personas ?? []).map((p) => [p.persona_id, p]));
        ...
        <div className="px-8 py-8 max-w-5xl mx-auto space-y-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-[#8B95A1] dark:text-[#6B7280] mt-1">
                run_id {result.run_id.slice(0, 8)} · 반응 {reactions.length}건 (QA 통과 {passed.length})
            ...
            <div className={cardCls}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                {[
                  ...
                  <div key={k}>
                    <p className="text-xs text-[#8B95A1] dark:text-[#6B7280]">{k}</p>
                    <p className="text-[#191F28] dark:text-[#F2F4F6] mt-0.5">{v || "—"}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className={cardCls}>
            <div className="flex items-center justify-between mb-3">
              ...
              {failed.length > 0 && (
                ...
                >
                  {showFailed ? "QA 통과분만 보기" : `QA 탈락 ${failed.length}건 포함`}
                </button>
              )}
            </div>
                ...
                return (
                  <div
                    ...
                    className={`border-l-2 pl-3 py-1 ${
                      r.qa_passed ? "border-[#3182F6]" : "border-[#F04452] opacity-60"
                    }`}
                  >
                    {/* 페르소나 기본 정보 */}
                    ...
                    {/* 반응 요약 */}
                    <div className="flex flex-wrap items-center gap-2 text-[11px] mb-1">
                      <span className="font-mono text-[#3182F6]">{aisasFunnel(r.aisas)}</span>
