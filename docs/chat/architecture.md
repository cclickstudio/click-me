...
작성 2026-06-23 · 상태 제안(설계 합의 전) · 기능 4-4(후순위, 핵심 3기능 완료 후 착수)
...
| P2-2 | **백그라운드 실행 + 완료 알림/재구독** — 실행 걸어두고 다른 대화 진행, 완료 시 채팅에 결과 푸시(SSE 재구독) | U4·R | P1-3·P0-3 | M |
...
- **오케스트레이터 패턴** — 매니지먼트의 LangGraph ReAct를 복제하되, 도구가 도메인 **서브에이전트**다(에이전트-of-에이전트). 단순 단일 그래프에 모든 도구를 다는 방식과 비교해, 도메인 응집·독립 테스트·점진 도입에 유리.
...
- **오케스트레이터 LLM** — 매니지 어시스턴트는 OpenAI, CLIO는 Gemini. 오케스트레이터를 어느 모델로? (도구 호출 안정성 기준 권장, 합의 필요)
- **위임 vs 직접 도구** — 도메인을 "서브에이전트(자체 LLM 루프)"로 부를지, "단일 도구 함수"로 부를지. 비용·지연 trade-off. 매니지는 이미 서브에이전트라 혼합이 자연스러움.
- **플로팅 ↔ 탭 상태 공유** — 같은 세션을 플로팅과 `/chat` 탭이 공유하는가, 분리하는가.
- **비용** — 채팅 한 번이 시뮬(최대 200 페르소나)·이미지 생성을 트리거. P3-2 가드 없이는 과금 위험.
