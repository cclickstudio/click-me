...
**✅ 완료(2026-06-26, 검증만·코드 변경 없음)** F5/L8 경로 end-to-end 동작 확인.
- **데이터(개인 DB)** 프로젝트에 `chat_long_term_memory` 8행(`user_profile_inferred`: 브랜드 '프리미엄 커피'·아라비카·성과중심, `gen_input`: 프리미엄 커피, `sim_input` 다수) + `chat_brand_profiles`(brand '프리미엄 커피'·tone '친근한'·target '20대 여성'·category '수분크림'·keywords) 존재.
- **검증(라이브 HTTP /chat/complete)** 제품/타깃을 **명시하지 않은** advise 질문("신제품 광고 처음 시작하는데 뭐부터 챙기면 좋을지 조언해줘") → CLIO(advise) 라우팅 → 응답에 **"20대 여성이라는 타겟에 맞는 톤"** 등장(`'20대 여성' in answer = True`). 즉 저장된 브랜드 프로파일 target_audience가 **언급 없이 응답에 반영** — 메모리 인지 성립. (`advise_node`가 `_format_brand`+`_format_ltm`+CLIO KB를 시스템 프롬프트에 주입하는 경로 동작 확인.)
- **관찰(설계 메모, 결함 아님)** 브랜드/LTM 프리앰블 주입은 **advise 라우트에서만** 일어난다. 도메인 라우트(sim/gen/management ask)는 각 서브에이전트가 도메인 데이터(특정 시뮬/생성 결과)를 도구로 조회하지 브랜드 프로파일을 주입하진 않음. 일반 대화(advise)에서 메모리 인지가 핵심이라 완료 기준 충족. (도메인 응답까지 브랜드 주입을 원하면 별도 개선 태스크 — 현 범위 밖.)
...
**✅ 완료(2026-06-26, 검증만·코드 변경 없음)** 가드 메커니즘 결정적 검증.
- **슬롯 스토어 로직(`runningJobs.ts`) 결정적 단위검증(6케이스)** — ① 초기 빈 상태 ② sim 점유 시 둘째 sim `getJobs().sim!==null`로 차단 ③ **sim+gen 동시는 둘 다 점유(허용)** ④ sim 해제 후 재실행 가능 ⑤ **STALE(31분 초과) 슬롯 자동 회수(null)** — 위젯 언마운트로 해제 콜백 누수 시 영구 점유 방지 ⑥ 10분(상한 내) 점유 유지. 전부 기대대로.
- **위젯 통합 인스펙션(양 도메인)** — [SimFormWidget.tsx:222](frontend/src/components/chat/SimFormWidget.tsx) `if(getJobs().sim) setErr('이미 다른 시뮬레이션이 진행 중이에요…')`·실행 시 `setSimJob(run_id)`(255)·완료/에러/정리 시 `setSimJob(null)`(109·147·259). [GenFormWidget.tsx:102](frontend/src/components/chat/GenFormWidget.tsx) 동형(`getJobs().gen`→`setErr('이미 다른 생성이 진행 중…')`·`setGenJob`(130) 점유·해제 72·143·156). 채팅 위젯과 `/simulation`·`/generations` 실행 페이지가 같은 전역 store 공유.
- **미검증(정직, 환경/비용)** **라이브 2-sim 동시 시도 토스트 픽셀**은 강제 안 함 — 가드는 프론트 전용 로직이라 위 단위검증으로 결정적으로 증명됨. 라이브 재현엔 실 시뮬 1회(LLM 비용)+다단계 폼(카테고리 드롭다운) 2회 작성이 필요한데, 확인 대상이 `setErr` 1줄 렌더라 비용 대비 가치 낮음. 슬롯 점유/차단/해제/STALE·sim+gen 독립은 전부 검증됨.
