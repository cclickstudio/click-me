...
## 2. 병행 진행 (워크트리 3분할 — 동시 개발)

> 남은 ⬜를 **충돌 없는 3개 워크트리로 나눠 동시에** 진행한다. 각 워크트리는 자기 브랜치에서 `/loop`로 돌리며, **세션 프롬프트는 분할 시점에 카드로 받아 그 세션에 붙여넣는다**(이 파일엔 공통 루프 프롬프트를 두지 않는다).
> **이 `tasklist.md`는 3 워크트리 공통 명세(읽기 전용)** — 태스크 명세(§4·4-S·4-N·4-L)는 참조하되 **§1 현황표 ✅·§5 진행 로그는 직접 수정하지 않는다**(동시 수정 시 머지 충돌). 진행 로그는 각자 `docs/chat/progress-<그룹>.md`(신규)에만 남긴다. 현황표·§5 통합은 머지 시점에 한 번에.

### 2-1. 분할표

| 워크트리 | 브랜치 | 그룹 | 태스크 | 소유(수정 허용) 범위 | 진행 로그 |
| --- | --- | --- | --- | --- | --- |
| **A** | `feat/chat-kb` | RAG/KB 적재 | R5 · R6 · R7 | `backend/domain/*/assistant/kb/`·`backend/domain/chat/kb/`의 `.md` + 해당 `kb_ingest`(매니지 신규 `.md` 시 그 도메인 `kb_ingest.py`의 `_SOURCE_META`만) | `progress-kb.md` |
| **B** | `feat/chat-be` | 채팅 백엔드 로직 | F5 · F6 · C1 | `backend/domain/chat/orchestrator.py`·`history.py`(·필요 시 `retriever.py`) | `progress-be.md` |
| **C** | `feat/chat-fe` | 프론트 채팅 위젯 | P5 · N4 · F10 | `frontend/src/components/chat/*` + F10 백엔드 키워드 엔드포인트는 `backend/api/routers/chat.py`에 **append-only** | `progress-fe.md` |

### 2-2. 충돌 방지 경계 (반드시 준수)

- **A vs B**: 둘 다 `backend/domain/chat/` 아래지만 A=`kb/*.md`(+ingest 실행), B=`*.py`로 **파일이 안 겹친다**. A는 `orchestrator/history/retriever` 손대지 말 것, B는 `kb/` 손대지 말 것.
- **B vs C**: B=backend, C=frontend. C만 예외로 `api/routers/chat.py`에 F10 엔드포인트를 추가하므로 **B는 `api/routers/chat.py`·`api/main.py`를 건드리지 않는다**.
- **공통부 금지**: 세 워크트리 모두 `core/models.py`(기존 컬럼)·`api/main.py` 라우터 등록·공용 `tools/`는 변경하지 않는다. 필요해지면 멈추고 progress에 로그 + 도연님께 조율 요청.
- **보류(외부 블로커, 어느 워크트리도 진행 X)**: 제너 403(V2·V3·G1~G4·F8·N3), 매니지 조율(S2·S5). 통합 QA·push는 C4(머지 후).

### 2-3. 머지 가이드

1. 각 워크트리 작업 종료 후 `feat/chat-doyeon`에서 `git merge feat/chat-kb` → `feat/chat-be` → `feat/chat-fe` 순으로 통합.
2. 파일이 그룹별로 갈려 코드 충돌 거의 없음. `progress-*.md`는 서로 다른 파일이라 충돌 없음.
3. 머지 후: 완료분을 §1 현황표 ✅·§5 진행 로그에 통합 반영 → **C4 전수 QA**(모든 기능 Claude Preview 회귀, 3역할) → 통과 시 push.
