...
- **🔵 시뮬 위젯 타이밍 수정(b7c8178)** — ① 결과 위젯이 `debate.start()`를 기다렸다 떠서 "토론 시작돼야 결과가 뜸"(준비 중 빈 상태처럼 보임) → 결과를 토론 시작 전에 먼저 append. ② 토론 위젯이 즉시 떠 "준비 중"을 길게 노출 → 토론 스트림 임시 구독해 **첫 발언(utterance) 때** 위젯 append(replay 안전). 검증(Preview): 반응 완료 직후 결과 표시·토론 위젯 미표시 → 첫 발언 시 토론 등장 → 완주·영속 확인.
