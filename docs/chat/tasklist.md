...
### C4 — 시뮬+채팅 전수 QA (대기업 시니어 QA 수준) 후 push

**역할/마인드셋** "이 사람 없으면 릴리스 못 한다"는 소리를 듣는 시니어 QA처럼 적대적·집요하게. 화면이 "떴다"는 통과가 아니다. 목표는 통과 도장이 아니라 **결함 발굴** — 시뮬레이터(4-1)+채팅(4-4)에서 "뭐가 실제로 안 되는지"를 미세한 것 하나까지 찾아 증거와 함께 기록.

**절대 원칙 (어기면 QA 실패)**
1. **빈 값·누락·이상 징후를 절대 그냥 넘기지 마라.** 위젯이 빈 채로 뜨거나 값이 0/null/"-"/"준비 중"으로 멈추거나 스피너가 안 끝나거나 숫자가 안 맞으면 → 다음으로 넘어가지 말고 그 자리에서 원인 규명(console_logs·network 응답본문·백엔드 로그·필요시 DB 직접 조회)해 결함으로 기록.
2. **"떴다"로 끝내지 마라.** 비동기는 완료까지 폴링 대기(시뮬/제너 수십 초~분). 결과 위젯·KPI·후보 이미지가 실제 값으로 채워졌는지 끝까지 본다.
3. **영속성·복원.** 진행 중 새로고침→스피너/진행 복원, 완료 후 새로고침→결과 복원, 세션 URL 직접 재진입→메시지·위젯 복원 셋 다.
4. **엣지·에러 직접 유발.** 빈/공백/초장문/특수문자 입력, 동시 실행, 네트워크 끊김(offline 토글), 키 한도를 실제로 만들어 에러 카드·재시도·가드가 도는지. "에러 안 남"이 아니라 "에러를 우아하게 처리"가 정상.
5. **콘솔 무에러.** 흐름 내내 console에 error/warning 없어야. 있으면 원인까지 기록.
6. **3역할 전부 반복.** admin·company·user 각각 재현. 역할 게이팅·자기 데이터 한정 확인. 역할별 차이는 역할 명시.

**체크리스트 (미세한 것까지)**
- **A. 채팅 기본** — /chat 진입·빈상태 칩, SSE 토큰 스트리밍 끊김 없음·전송중 중복차단, 사이드바 목록/정렬/하이라이트·자동제목(F13), 세션 생성/삭제·라우팅 정합, 새로고침·URL 재진입 복원, 플로팅 챗 노출/숨김·알림 벨/배지(N5), 슬래시 전부 1회씩·잘못된 인자 처리, IME 조합중 Enter 오전송·Shift+Enter·초장문·빈 메시지 차단.
...
- **D. 메모리/RAG/능동** — 수 회 실행 후 명시 언급 없이 브랜드·제품군 인지(롱텀), 광고 용어 질문→CLIO KB 근거+인용 칩, 제너 전용 세션에 시뮬 선제 알림 오염(N6).
- **E. 권한/게이팅(3역할)** — 비로그인 차단, USER 타 프로젝트/조직 403·자기 데이터 정상, admin 전용 화면·API에 USER/COMPANY 차단.

**결함 처리** 발견 즉시 `docs/chat/qa-report-<날짜>.md`에 기록: 제목 / 심각도(S0 블로커·S1 심각·S2 보통·S3 사소) / 클릭 단위 재현 절차 / 기대 / 실제 / 증거(스크린샷·콘솔·네트워크·DB 조회) / 추정 원인(파일:라인). **S3(한 줄 수정)만 그 자리 고치고 `fix:` 커밋**, S0~S2·불확실은 리포트만. 추측 금지 — 재현·관찰한 것만, 못 한 검증은 "미검증" 명시.
