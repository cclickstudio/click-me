...
> 분석가 토론·RAG·벤치마크·진단/권고는 분석팀 소유라 제외(§경계 주의 참조). 최종 갱신 2026-06-22.
...
| GENERATION | `docs/simulation/Persona/PERSONA_GENERATION_STRATEGY (1).md` |
...
도메인 `README.md §4`는 "광고해석·루브릭·반응 실 LLM 어댑터는 아직 mock(P4 예정)"이라 적혀 있으나,
`wiring.py`는 이미 `GeminiAdInterpreter`·`GeminiRubricEvaluator`·`GeminiReactionEngine`을 mock 폴백 없이 연결한다.
즉 **P4(실 LLM 연결)는 완료**됐고 README가 stale이다.
...
1. **A-1 (3-모드 UX)** — 제품 가치, 엔진 재사용으로 저비용.
2. **A-4·A-5 (재현 모드·QA)** — 외부 데이터 없이 바로 구현 가능.
3. **A-2·A-3 (calibration·검증 확대)** — KOBACO 베이스라인 수치 확보 후.
4. 나머지(B·C·D)는 데이터 확보·발표 후로.
