...
- **공유 해석 1회 → 페르소나별 조건화.** `interpret_ad`가 비싼 vision을 1회만 내고 `visual_elements`·`brand_era`를 `structured_analysis`에 담으면, `react` 워커는 그 공유 '사실'에 페르소나 나이·프로필을 곱해 다르게 반응한다(Tier 1 세대 게이팅 + §4-b salience). vision을 N배로 늘리지 않고 "같은 자극, 다른 주의·친숙도"를 만든다(페르소나별 이미지 해석 검토·페르소나 세대 지식 전략).
