...
| # | 노드 | 하는 일 | 호출 어댑터(주입) |
|---|---|---|---|
| 1 | `interpret_ad` | ① VLM 감지(선언 미주입) → ② 의도 교차검증(정합 채점) → ③ `intent_mismatch`·`mismatch_detail` 파생 → ④ **브랜드 인지율(Tier3) 룩업해 `structured_analysis.awareness_by_age` 부착**(스키마 무변경, 미수록 시 폴백, 작업1). 감지 콜이 `visual_elements`(§4-a)·`brand_era`(Tier 2)도 `structured_analysis`에 함께 추출 → 전 페르소나 공유. `ad` + `rubric_scores` 동시 산출 | `interpreter.interpret` + `rubric.evaluate(ad, request)` + `lookup_brand_awareness` |
