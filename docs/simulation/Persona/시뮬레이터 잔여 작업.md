...
| 약칭 | 실제 파일 |
| --- | --- |
| IMPROVEMENT | `docs/simulation/Persona/광고 시뮬레이터 보완 과제.md` |
| GENERATION | `docs/simulation/Persona/페르소나 생성 전략.md` |
| COHORT_KNOWLEDGE | `docs/simulation/Persona/페르소나 세대 지식 전략.md` |
| COHORT_TIER3 | `docs/simulation/Persona/브랜드 인지도 실데이터 전략(Tier3).md` |
| BATCH_API | `docs/simulation/Persona/페르소나 반응 LLM 비용 전략.md` §5~10 (구 BATCH_API_STRATEGY.md 병합) |
| VLM | `docs/simulation/Persona/페르소나별 이미지 해석 검토.md` |
| ANALYSIS | `docs/simulation/Persona/분석 에이전트(토론) 전략.md` |
| Data_Collection | `docs/simulation/Persona/데이터 확보처 가이드.md` |

(P1-3·P2-5 등은 해당 문서 §2 우선순위 번호. 예: IMPROVEMENT P2-4 = 광고 시뮬레이터 보완 과제.md §2 P2의 4번)
...
- **SSR 점수화 opt-in ✅** — `SIMULATION_SCORING=ssr`이면 반응 체인을 `SSRScoringReactor`로 감싸 구매의도·신뢰도만 임베딩 분포로 재산정(기본은 `llm`, 즉 OFF). 2026-07-08 하루 기본 ON으로 전환했다가(`4ffa4929`) 다음날 원복(`c5f68f11`) — 현재는 다시 opt-in. 초기 설계의 노출(`exposure.py`)·숙고(`deliberation.py`) 2단계는 어디서도 안 쓰이는 죽은 코드.
...
| # | 작업 | 출처 | 비고 |
| --- | --- | --- | --- |
| 6 | **OCEAN 연령밴드별 유형비율** — 현재 40대+도 20대와 동일 `type_proportions`. 연령×유형 교차표는 Nature 미수록(저자요청만). BFI-K 고령자 규준은 확보 완료(raw 저장)이나 직접주입 금지·정성 prior. → 데이터 확보처 가이드 §단계 2-α | IMPROVEMENT P2-4 | 교차표 미공개 / BFI-K 확보됨 |
...
> **2026-06-22 프레임워크 패스(10·11·13·14)** — 5종 모두 실데이터가 게이트라 "코드 프레임워크 + graceful fallback"을 먼저 구현했다. 데이터 0 상태에선 현재 동작을 그대로 보존(회귀 0)하고, 사용자가 데이터를 주입하면 즉시 활성화된다. 데이터 수집 리스트·라이선스는 `데이터 확보처 가이드.md` 참조. 12(40+ OCEAN)는 B-6에서 OSF 실데이터로 이미 해결, MDIS 상관행렬만 남았다.
