...
- [x] (C 시) 옵션 선택 UX — **옵션 버튼** 확정·구현. meta의 옵션 목록(`OPTION_POOLS`,
  ...
  - ~~버튼은 1회용 비활성화하지 않고 유지~~ → **선택 시 나머지 옵션 비활성화**로 변경
    (2026-07-04 실사용 피드백 — 중복 전송·혼란 방지. 선택 후 다른 조치는 [기타] 자유 입력,
    재선택 UI는 후속 검토). 클릭 시 대화 하단 자동 스크롤.
  구현: `chat/RemediationOptionsWidget.tsx` 위젯(Task 13).
