...
| `AUDIENCE_TOO_NARROW` | `audience_size` 30k + 다중일 누적 | reach 포화(≈28.7k) → frequency 3+ → impressions↓ |
...
### 4.6 Mock 수치의 근거와 한계 (발표·검증용 정직 공개)

**근거 (각 값이 어디서 왔는가)**

| 파라미터 | 값 (코드) | 근거 |
|---|---|---|
| `CPM_ANCHOR_KRW` | ₩10,800 | 한국 실측 lebesgue $5.80(₩7,800)·AdAmigo $10.20(₩13,770)의 중앙값 (§3.2) |
| `CPM_NORMAL_RANGE_KRW` | ₩7,800~13,800 | 위 두 출처의 하한~상한 |
| `BASE_CTR` | 1.7% | AdAmigo 2026 트래픽 목표 CTR 1.71% (§3.2) |
| `cpc` (파생) | ≈₩635 | `cpm/(ctr×1000)` — 트래픽 CPC $0.70 범위 내 |
| `AUDIENCE_SIZE` | 1,750,000 | Meta delivery_estimate `estimate_mau` 기반 |
| 좁은 모수 + 다중일 누적 | 30,000 / 누적 = 모수×3(≈9.5만) | 좁은 모수는 수일 누적으로 reach 포화 → frequency 3+ (피로 통념). 관측일=성숙 캠페인 일자. 실측: cum_reach 28.7k, frequency 3.31 |
| 빈도 피로 임계 | 3.0 | frequency 3+ = 도달 피로 업계 통념 |
| 일중 곡선 | 이중 봉우리 | 인스타 점심(12~13시)·저녁(20~23시) 피크 통상 패턴 (§4.3) |
| 고장 강도 | cpm×1.5(BID_LOSS)·ctr×0.3(QUALITY) 등 | 감지 가능·오분류 방지로 calibrate한 값 |

**한계 (반드시 함께 공개)**

1. **시뮬↔실측 상관 미검증 (가장 중요)** — Mock은 평가 하니스다. 가상 지표가 실제 캠페인 성과와 상관하는지 아직 검증되지 않았다. 실측 데이터 누적 후 calibration으로 해금한다 (PRD §7). "정답"은 *주입한 고장을 맞혔는가*이지 현실 일치가 아니다.
2. **CPM 절대값 신뢰구간이 넓다** — 한국 출처가 $5.80 vs $10.20로 **2배 차이**. 서드파티 집계(메타 국가별 CPM 비공개)이고 이커머스/전 목표 혼합이다. *방향성(한국<미국)은 일관*하나 절대값은 출처 의존적이라 중앙값을 택했다.
3. **CTR은 트래픽 목표 한정** — 1.71%는 트래픽 목표 기준. 다른 목표(전환 등)·지면(Reels/Feed/Stories)에선 달라진다.
4. **audience-fatigue는 데모 파라미터** — 모수 30,000·누적 모수×3(≈10일치)은 "피로가 관측되는 성숙 캠페인 일자"를 보이려 택한 값이다. 실 캠페인은 모수·기간이 가변이고, reach는 지수 포화 모델 가정이라 실 Meta reach 곡선과 다를 수 있다.
5. **고장 강도는 벤치마크가 아니다** — cpm×1.5 등은 "감지 가능한 증상"을 만드는 calibration값이지 실측 고장 크기가 아니다.
6. **일중 곡선은 정성적** — 이중 봉우리는 통념이고, 시간별 가중치는 벤치마크 수치가 없는 가정이다.
