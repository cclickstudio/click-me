...
|---|---|---|---|
| `cpm` | ₩9,000* | ₩6,000 ~ ₩18,000 | Feed $10~16 / Reels $4~8 |
| `inline_link_ctr` | 1.7% | 0.8% ~ 2.5% | 트래픽 CTR 1.71% |
| `cpc` (파생 검증용) | ₩1,000 | ₩600 ~ ₩2,400 | 트래픽 CPC $0.70 |
| `frequency` | — | 1.0~2.5 정상 / 3+ 피로 | 도달 포화 |

> *한국 실데이터($5.80 ≈ ₩7,800) 반영 시 `cpm` 중심값을 **₩7,000~₩8,000**, 범위 ₩6,000~₩14,000으로 낮추는 게 더 방어 가능.
...
| ~~전환·ROAS~~ → **해제(06-20)** · 다중 플랫폼·LIVE 집행 | 전환·ROAS는 In scope(전환 일반화·추정 ROAS·목표 기준 이상판정). 다중 플랫폼·LIVE는 §7 **Won't** 유지 |
