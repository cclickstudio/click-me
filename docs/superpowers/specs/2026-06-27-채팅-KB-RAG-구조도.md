...
## 14. 작업 재개 프로토콜 (5시간 초기화·세션 중단 대비)

> 사용 한도 5시간 초기화나 컨텍스트 압축으로 작업이 끊겨도, 새 세션이 **묻지 않고 알아서 이어서** 진행하기 위한 절차. 상태의 단일 출처는 **Plan 문서(`2026-06-27-채팅-통합-계획.md`)의 「작업 로그」**.

**중단 시:** 진행 중이던 최소 태스크를 안전 지점(커밋 또는 깨끗한 working tree)까지만 마무리하거나, 불가하면 그 지점을 Plan 작업 로그·미검수에 한 줄 남긴다. 그리고 **잠시 멈춘다**(재개 신호 대기).

**재개 시 (새 세션이 자동 수행):**
1. **상태 복원** — Plan 「작업 로그」를 읽어 마지막 ✅ 완료 갭을 찾는다. `git log --oneline -10`으로 실제 커밋 상태와 대조(로그와 커밋 불일치 시 커밋이 진실).
2. **다음 태스크 결정** — 의존 순서(§13: G1→G5→G4→G2→G3→G6→G7, 메모리 M3→M1→M2→M6)에서 **마지막 완료 다음**을 집는다. 완료된 갭은 다시 하지 않는다.
3. **실행 규칙 준수** — Plan 「실행 규칙」 그대로: 최소 단위 · DB/코드/웹/FE 검수 · 테스트 3회 · 환각 4지표 개선전/후 · Ruff · 의미단위 커밋 · 로그 기재.
4. **검증 후 진행** — 3회 통과 + 진짜 이상 없을 때만 다음. 실패 시 「테스트 실패 시 대응 정책」 적용.
5. **묻지 않는다** — 판단 필요 시 합리적 결정으로 우선 진행하고 근거를 로그에 남긴다(자율). 단 되돌리기 어려운 것(공유 브랜치 푸시·DB 스키마)은 미검수에 표시하고 안전 지점에서 멈춘다.

**재개 안전장치:** 중단 직전 미완료 코드가 lint/구문 깨진 상태면, 재개 시 **먼저 그 파일을 안전 상태로 복구**(완성 또는 되돌림)한 뒤 다음 태스크로. 깨진 채 새 작업 쌓기 금지.

> 한 줄 요약: **"Plan 작업 로그 = 체크포인트. 멈추면 거기 남기고, 켜지면 거기서 이어간다."**
