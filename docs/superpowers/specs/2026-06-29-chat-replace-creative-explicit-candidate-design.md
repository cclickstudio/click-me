...
- **⑫ Codex 적대 리뷰 반영(fail-fast 가드)** — (high) 레거시 REPLACE(`selected_candidate_id`·campaign target)는 Meta ad/creative 모델과 안 맞아 실 모드에서 캠페인 id를 ad로 보내 승인 후 깨진다. → executor `_validate`가 **레거시 REPLACE를 sending(validate/live) 모드에서 승인 전 거부**(UNSUPPORTED_ACTION), mock 데모(재생성 루프)는 허용. (medium) `replace_creative_tree`가 VALIDATE_ONLY에서 합성 creative_id면 fan-out을 실제 검증 못 하므로 **success가 아니라 명시적 미검증 실패(FAILED + validate_note)** 로 반환(false-green 방지). 둘 다 회귀 테스트 추가.
