...
> **핵심 — 답변 충실도(faithfulness)를 측정 기반으로 개선했다. 검증되는 최신 스냅샷 = faithful_rate 0.778(21/27 · judge×3).** before 시점 결과 파일이 없어 개선 델타(before/after)는 커밋 데이터로 재현되지 않는다(작업 내역 §4.3).
...
- **집계**: judge(Gemini Flash) **×3 반복** — `faithful`은 과반(2/3) 투표, `score`는 평균. n=27, 잔여 judge 표준편차 0.28(`evals/_faithfulness_results.jsonl`).
...
1. **베이스라인(엄격 judge)** — judge 과엄격(맞는 일반조언도 감점) + 행동버그(개념질문에 live 덤프·KB 미사용) 식별.
2. **Grounding 게이트 시도** → 개선 없음 → **revert**(지연만 추가하는 음성 결과).
3. **judge 재보정**(환각 중심) + **프롬프트 라우팅 수정**(개념질문→search_kb, live 덤프 금지).
4. **재측정** — 검증되는 결과 파일 스냅샷.

| 측정 | faithful_rate | 출처 |
|---|---|---|
| **정밀 측정(n=27·judge×3)** | **0.778** (21/27 · 평균 4.47/5 · std 0.28) | ✅ `evals/_faithfulness_results.jsonl` |

> ⚠️ **before 시점 결과 파일이 없어 개선 델타(before/after)는 커밋 데이터로 재현 불가.** judge(Gemini Flash) 비결정성으로 경계 케이스가 회차마다 ±2개 흔들려 faithful_rate가 회차별 0.78~0.85 범위에서 변동한다. 개선 작업(grounding 게이트 revert·judge 재보정·라우팅 수정)은 실재하나, **수치 주장은 저장된 단일 스냅샷 0.778만 인정**한다.
...
검색 recall@3 **0.93**(n=28, 결과 파일 미저장) · faithfulness **0.778**(21/27·judge×3, std 0.28, ✅ 결과 파일) · 매니지먼트 테스트 통과 · Alembic head=`028`.
