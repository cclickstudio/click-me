...
- **스캐너 계약** — 신형은 `(findings, normals)` 튜플 반환. finding meta에 `anomaly_type` 힌트 포함. normals = `[{tenant_id, campaign_id, anomaly_type}]`(성공 조회+정상만 — fail-closed). `run_scan`은 튜플/구형 list 모두 수용, normals 있고 `hasattr(sink, "reconcile")`면 호출.
