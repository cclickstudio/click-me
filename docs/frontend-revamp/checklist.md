...
- [x] "지금 주목할 것": 주의 필요 항목 alert 카드(semantic). — **실데이터 기반으로 완성**: 실패한 생성(danger)·진행 중 시뮬(info)을 semantic 카드로 KPI 아래 프롬프트 위치에 표기(점진적 공개 — 항목 있을 때만). test 로그인 실렌더 검증(생성 실패 2건 danger 카드). ⚠️ Meta 이상감지·리밸런스·승인대기는 이 환경에 Meta 연동 없어 범위 밖 — 대신 앱 자체 활동 신호로 "주목할 것" 의도 충족.
